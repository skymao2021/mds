...
function isMatchingTargetPath(filename, targetPath) {
    if ((0, utils_1.isDynamicPattern)(targetPath)) {
        return (0, utils_1.isMatch)(filename, targetPath);
    }
...
module.exports = (0, utils_1.createRule)({
    ...
    create(context) {
        ...
        function computeGlobPatternPathValidator(absoluteFrom, zoneExcept) {
            let isPathException;
            const isPathRestricted = (0, utils_1.fileMatcher)(absoluteFrom);
            const hasValidExceptions = zoneExcept.every(it => (0, utils_1.isDynamicPattern)(it));
            if (hasValidExceptions) {
                const exceptionsMm = zoneExcept.map(except => (0, utils_1.fileMatcher)(except));
                isPathException = (absoluteImportPath) => exceptionsMm.some(mm => mm(absoluteImportPath));
            }
        ...
        const makePathValidators = (zoneFrom, zoneExcept = []) => {
            ...
            const areGlobPatterns = allZoneFrom.map(it => (0, utils_1.isDynamicPattern)(it));
