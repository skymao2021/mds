    ...
    static parse(filepath, content, context) {
        ...
        function processSpecifier(s, n, m) {
            ...
            switch (s.type) {
                ...
                default: {
                    if ('local' in s) {
                        local = (0, get_value_1.getValue)(s.local);
                    }
                    else {
            ...
            if ('exported' in s) {
                m.reexports.set((0, get_value_1.getValue)(s.exported), {
                    local,
...
function makeContextCacheKey(context) {
    ...
    let hash = (0, stable_hash_1.stableHash)(settings) +
        (0, stable_hash_1.stableHash)(languageOptions?.parserOptions ?? parserOptions);
    if (languageOptions) {
        hash +=
            String(languageOptions.ecmaVersion) + String(languageOptions.sourceType);
    }
    hash += (0, stable_hash_1.stableHash)(parserPath ?? languageOptions?.parser?.meta ?? languageOptions?.parser);
    return hash;
}
