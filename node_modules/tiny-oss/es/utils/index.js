...
function getContentMd5(buf) {
  ...
  var md5Buf = hexToBuffer(md5(bytes));
...
function getSignature(options) {
  ...
  var text = data.join('\n');
  var hmac = new Digest.HMAC_SHA1();
  hmac.setKey(accessKeySecret);
  hmac.update(text);
  var hashBuf = new Uint8Array(hmac.finalize());
  var signature = base64js.fromByteArray(hashBuf);
  return signature;
}

export { unix, blobToBuffer, assertOptions, getContentMd5, getCanonicalizedOSSHeaders, getCanonicalizedResource, getSignature };