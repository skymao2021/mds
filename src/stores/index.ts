...
export const useStore = defineStore(`store`, () => {
  ...
  // 导入 Word 文档并转换为 Markdown
  const importWordContent = (file?: File) => {
    if (file) {
      // 如果直接传入文件，处理该文件
  ...
  // 处理 Word 文件并转换为 Markdown
  const processWordFile = (file: File) => {
    const reader = new FileReader()
    reader.onload = (event) => {
      ...
      import('mammoth').then((mammoth) => {
        mammoth.default.convertToMarkdown({ arrayBuffer })
          .then((result) => {
            // 设置编辑器内容为转换后的 Markdown
            editor.value!.setValue(result.value)
            toast.success(`Word 文档转换成功`)
